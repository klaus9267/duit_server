...
import duit.server.domain.subscription.dto.JobKeywordSubscriptionResponse
...
@Component
class JobKeywordSubscriptionFactory(
    private val subscriptionRepository: SubscriptionRepository,
) : SubscriptionFactory {

    override val type = SubscriptionType.JOB_KEYWORD
    ...
    override fun create(user: User, request: SubscriptionCreateRequest): Subscription {
        val keyword = request.keyword?.takeIf { it.isNotBlank() }
            ?: throw IllegalArgumentException("JOB_KEYWORD 구독에는 keyword 가 필요합니다")
    ...
    override fun toResponse(subscription: Subscription): SubscriptionResponse =
        JobKeywordSubscriptionResponse(
            id = subscription.id!!,
